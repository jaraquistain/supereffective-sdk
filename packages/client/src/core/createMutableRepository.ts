...
import type { Entity, EntityUpdate, MutableRepository, MutableRepositoryDataProvider, RepositoryConfig } from './types'
  ...
  const repo: MutableRepository<R> = {
    ...
    async updateMany(updatedEntities: EntityUpdate<R>[]) {
      ...
      const updatedEntitiesResult = allEntities.map((entity) =>
        updatedEntitiesMap.has(entity.id) ? { ...entity, ...updatedEntitiesMap.get(entity.id) } : entity,
      )
